...
class MysensorsDebugDecode {
    constructor() {
        this.rprefix = '(?:\\d+ )?(?:mysgw: )?(?:Client 0: )?';
        this.match = [
            ...
            { re: 'SGN:SKP:MSG CMD=(\\d+),TYPE=(\\d+)', d: 'Message with command  $1  and type  $1  does not need to be signed' },
        ...
        for (let i = 0, len = this.match.length; !found && i < len; i++) {
            ...
            if (r.re instanceof RegExp) {
                if (r.re.test(msg)) {
                    ...
                    msg = msg.replace(/{command:(\d+)}/g, (__, m1) => mysensors_types_1.mysensor_command[m1]);
                    msg = msg.replace(/{pt:(\d+)}/g, (__, m1) => mysensors_types_1.mysensor_payload[m1]);
    ...
    type(cmd, type) {
        switch (cmd) {
            ...
            case mysensors_types_1.mysensor_command.C_SET:
                return mysensors_types_1.mysensor_data[type];
            case mysensors_types_1.mysensor_command.C_INTERNAL:
                return mysensors_types_1.mysensor_internal[type];
            case mysensors_types_1.mysensor_command.C_PRESENTATION:
                return mysensors_types_1.mysensor_sensor[type];
            case mysensors_types_1.mysensor_command.C_STREAM:
                return mysensors_types_1.mysensor_stream[type];
        }
        return '';
    }
}
exports.MysensorsDebugDecode = MysensorsDebugDecode;
